...
public class CommerceDBHelper extends SQLiteOpenHelper {
    ...
    public boolean updateQuantity(int quant, int id) {
        SQLiteDatabase commerceDatabase = null;
        Cursor cursor = null;
        try {
            // Open the database
            commerceDatabase = getReadableDatabase();

            // Query the available quantity of the product
            String query = "SELECT quantity FROM product WHERE prodid = ?";
            cursor = commerceDatabase.rawQuery(query, new String[]{String.valueOf(id)});

            if (cursor != null && cursor.moveToFirst()) {
                int availableQuantity = cursor.getInt(0);

                // Check if the requested quantity is less than or equal to available quantity
                if (availableQuantity >= quant) {
                    // Update the quantity in `unconfirmes_orders`
                    ContentValues row = new ContentValues();
                    row.put("quantity", quant);

                    commerceDatabase = getWritableDatabase();
                    int rowsUpdated = commerceDatabase.update(
                            "unconfirmes_orders",
                            row,
                            "proid = ?",
            ...
            // Always close the cursor and database to prevent resource leaks
            if (cursor != null) cursor.close();
            if (commerceDatabase != null && commerceDatabase.isOpen()) commerceDatabase.close();
        }
    }
    ...
    public void deleteProduct(int id){
        ...
        // Now remove any references to this product from the unconfirmed orders table
        commerceDatabase.delete("unconfirmes_orders", "proid='" + id + "'", null);
        commerceDatabase.close();
    }
    ...
    public boolean updateUser(int userId, String name, String username, String password, String gender, String birthdate, String job) {
        ...
        int rowsAffected = db.update("customer", values, "custid = ?", new String[]{String.valueOf(userId)});
